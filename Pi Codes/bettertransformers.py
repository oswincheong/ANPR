...
# Parameters
batch_size = 2  # Adjusted for better latency on Raspberry Pi 5
epochs = 20
...
# Load the pre-trained model and apply BetterTransformer
trained_model = VisionEncoderDecoderModel.from_pretrained('checkpoint-' + str(3300)).to(device)
trained_model.to_bettertransformer()
...
def eval_new_data(
    ...
    ground_truth_path=r'/home/oswin/Documents/ANPR/Dataset/test.txt',
    batch_size= batch_size # Adjusted batch size based on memory constraints
):
    total_correct = 0
    total_samples = 0
    total_latency = 0
